...
class STTTranslator:
    ...
    async def translate_to_lang(self, text: str, target_lang: str) -> str:
        ...
        system_prompt = (
            f"Interprete simultaneo de television en directo. "
            f"Traduce al {lang_name} de forma natural, concisa y directa para rotulos televisivos. "
            "Devuelve UNICA y EXCLUSIVAMENTE una sola linea con la traduccion. Sin comentarios, sin comillas."
        )

        if self.groq_client:
            # 1. Intentar con qwen/qwen3.8-27b
            try:
            ...
            # 2. Respaldo ultrarrápido con allam-2-7b
            try:
                response = await self.groq_client.chat.completions.create(
                    model="allam-2-7b",
                    messages=[
